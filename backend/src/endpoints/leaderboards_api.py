...
def get_filtered_leaderboard_entries(entries: List, current_user_id: Optional[int]) -> tuple:
    ...
    if current_user_id is None:
        return entries[:10], False

    # Find user's entry
    user_entry = None
    user_index = None

    logger.debug(f"Searching for user_id {current_user_id} in {len(entries)} entries")
    logger.debug(f"Entry user_ids: {[entry.user_id for entry in entries[:15]]}")  # Log first 15 user IDs

    for idx, entry in enumerate(entries):
        if entry.user_id == current_user_id:
            user_entry = entry
            user_index = idx
            logger.debug(f"Found user at index {idx} (rank {entry.rank})")
            break
    ...
    if user_index == 10:
        result = list(entries[:12])  # Top 10 + 11 + 12
    ...
    if user_index == 11:
        end_idx = min(13, len(entries))  # Top 10 + 11 + 12 + 13 (if exists)
        result = list(entries[:end_idx])
    ...
    result = list(entries[:10])  # Top 10

    # Add entry before user (if exists)
    if user_index > 0:
        result.append(entries[user_index - 1])

    # Add user's entry
    result.append(user_entry)

    # Add entry after user (if exists)
    if user_index < len(entries) - 1:
        result.append(entries[user_index + 1])
...
def get_leaderboards(current_user_id: Optional[int] = None, db: Session = Depends(get_db)):
    ...
    try:
        ...
        for comp in competitions:
            ...
            # Filter entries based on user position
...
def get_all_algotime_leaderboard_entries(db: Session = Depends(get_db)):
    ...
    try:
        entries = db.query(AlgoTimeLeaderboardEntry).order_by(
