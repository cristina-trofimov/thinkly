...
const ConsoleOutput = ({logs}: {logs: Judge0Response[]}) => {
  ...
  return (
    <div data-testid="ConsoleOutput"
        className='w-full h-full flex flex-col-reverse gap-2 overflow-y-scroll overscroll-x-scroll'
    >
        {/* <div className='flex flex-col-reverse gap-2 overflow-y-scroll overscroll-x-scroll' > */}
            {logs.map((log, idx) => {
                const stat = log.status.description.toLowerCase() === "accepted" ? "Passed" : "Failed"
