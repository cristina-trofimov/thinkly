...
import { type CreateCompetitionDialogProps, type EmailPayload } from "../interfaces/CreateCompetitionTypes";

type QuestionDTO = { id: string | number; title: string; difficulty?: string | null };

const API_BASE = import.meta.env.VITE_API_BASE ?? "http://127.0.0.1:8000";
const EMAIL_API_BASE = "http://127.0.0.1:8001";
...
export default function CreateCompetitionDialog({ open, onOpenChange }: CreateCompetitionDialogProps) {
    const [formData, setFormData] = useState({
        name: "", date: "", startTime: "", endTime: "", questionCooldownTime: "", riddleCooldownTime: "",
    });
    const [emailData, setEmailData] = useState({
        to: "", subject: "", text: "", sendAtLocal: "", sendInOneMinute: false,
    });
    ...
    const toggleRiddle = (id: number) =>
        setSelectedRiddles(prev => prev.includes(id) ? prev.filter(x => x !== id) : [...prev, id]);

    const validateForm = () => {
        if (!formData.name.trim() || !formData.date || !formData.startTime || !formData.endTime) {
            setValidationError("Incomplete general information."); return false;
        }
        const dt = new Date(`${formData.date}T${formData.startTime}`);
        const now = new Date(); now.setSeconds(0, 0);
        if (dt.getTime() <= now.getTime()) { setValidationError("The competition must be scheduled for a future date and time."); return false; }
        if (selectedQuestions.length === 0) { setValidationError("Please select at least one question."); return false; }
        if (selectedRiddles.length === 0) { setValidationError("Please select at least one riddle."); return false; }
    ...
    const handleSubmit = async () => {
        ...
        if (emailData.to.trim()) {
            ...
            if (toList.length) {
                ...
                try {
                    const res = await fetch(`${EMAIL_API_BASE}/send-email`, {
                        method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(payload),
                    });
                    const body = await res.json().catch(() => ({}));
                    if (!res.ok) console.error("Email send failed:", body?.error || `HTTP ${res.status}`);
                    else console.log(sendAt ? "Email scheduled ✅" : "Email sent ✅");
                } catch (e: any) {
        ...
        setSelectedQuestions([]); setSelectedRiddles([]);
    };
        ...
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent className="sm:max-w-[600px] max-h-[90vh]">
                <DialogHeader>
                    ...
                    {validationError && (
                        <p className="text-red-500 text-sm font-medium border border-red-300 p-2 rounded-md bg-red-50">⚠️ {validationError}</p>
                    )}
                </DialogHeader>
                ...
                <div className="grid gap-6 py-4 overflow-y-auto max-h-[60vh] pr-2">
                                        ...
                                        <div key={id} className="flex items-center gap-3 p-3 border-b last:border-b-0 hover:bg-gray-50">
                                            ...
                                            <div className="flex-1">
                                                <div className="flex items-center gap-2">
                                                    ...
                                                    <span className={`text-xs px-2 py-0.5 rounded ${label === "Easy" ? "bg-green-100 text-green-700" :
                    ...
                    <div className="grid gap-4">
                        ...
                        <div className="grid gap-2">
                            ...
                            <Input id="emailTo" value={emailData.to} onChange={(e) => setEmailData({ ...emailData, to: e.target.value })} placeholder="alice@example.com, bob@example.com" />
                        </div>
                        <div className="grid gap-2">
                            <Label htmlFor="emailSubject">Subject</Label>
                            <Input id="emailSubject" value={emailData.subject} onChange={(e) => setEmailData({ ...emailData, subject: e.target.value })} placeholder="Competition announcement" />
                        </div>
                        <div className="grid gap-2">
                            <Label htmlFor="emailText">Message</Label>
                            <Textarea id="emailText" rows={4} value={emailData.text} onChange={(e) => setEmailData({ ...emailData, text: e.target.value })} placeholder="Write your message..." />
                        </div>
                        <div className="grid gap-4 md:grid-cols-2">
                            <div className="flex items-center justify-between rounded-md border p-3">
                                ...
                                <Switch id="sendInOneMinute" checked={emailData.sendInOneMinute} onCheckedChange={(checked) => setEmailData({ ...emailData, sendInOneMinute: checked })} />
                            </div>
                            <div className="grid gap-2">
